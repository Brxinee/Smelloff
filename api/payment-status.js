...
export async function checkBankUpiStatus(order) {
  ...
  try {
    ...
    // Standard bank / acquirer status checks
  ...
  try {
    ...
    if (!tokenValid && !phoneValid) {
      return res.status(403).json({
        error: 'Order ownership verification failed. Valid order token or phone required.'
      });
    }
    ...
    // If order is upi_pending, optionally check bank/merchant status API
    ...
    // Default status response (still pending)
    return res.status(200).json({
      ok: true,
      orderId: orderCode,
      status: order.status,
      ...
      paymentMethod: order.payment_method,
      amount: order.amount ? order.amount / 100 : 229,
      message: order.status === 'upi_pending'
        ? 'Awaiting UPI payment confirmation'
        : `Order is in status '${order.status}'`
    });
