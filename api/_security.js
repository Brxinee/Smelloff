...
export function clientIp(req) {
  ...
  return pick(req.headers['cf-connecting-ip']) || pick(req.headers['true-client-ip']) || pick(req.headers['x-forwarded-for']) || req.socket?.remoteAddress || 'unknown';
}
...
export function checkRateLimit(key, limit = 10, windowMs = 10 * 60 * 1000) {
  ...
  if (!bucket || now >= bucket.resetAt) {
    ...
    if (rateLimitBuckets.size > 5000) for (const [k, v] of rateLimitBuckets) if (now >= v.resetAt) rateLimitBuckets.delete(k);
    return true;
  }
...
export function getSecuritySecret() {
  const secret = process.env.ORDER_SECURITY_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.ADMIN_SECRET;
...
export function generateOrderToken(orderCode, phone) {
  ...
  return crypto.createHmac('sha256', secret).update(`${cleanOrderCode}:${cleanPhone}`).digest('hex').slice(0, 32);
}
