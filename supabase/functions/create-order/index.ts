...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const clientUnitPrice = Number(source.price);
    if (clientUnitPrice !== UNIT_PRICE_RUPEES) {
    ...
    if (codFeeRupees > 0) {
      await supabase.from("orders").update({ cod_fee: codFeeRupees * 100 }).eq("id", data.id);
    }
    if (fbp || fbc || clientIp || clientUa || eventSourceUrl) {
      await supabase.from("orders").update({
        fbp, fbc, client_ip: clientIp, client_ua: clientUa, event_source_url: eventSourceUrl,
      }).eq("id", data.id);
    }
