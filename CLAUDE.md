...
- `/account` — customer account (phone-OTP login). Order history, live tracking timeline, self-serve cancellation before dispatch, saved profile/address. noindex + `Disallow` in robots.txt, never in the sitemap. **Checkout is account-gated**: `/odorstrike` shows a verify-your-number gate before the order form, and `create-order` takes the customer's phone from the verified JWT claim (never the request body). Session client is `assets/js/auth.js` — hand-rolled against the Supabase REST endpoints because the CSP is `script-src 'self'` and a CDN `supabase-js` would be blocked. OTP SMS goes through `supabase/functions/send-sms-hook` to an Indian provider (MSG91/2Factor/Fast2SMS, `SMS_PROVIDER` env) instead of Twilio — ~₹0.15 vs ~₹3 per message. Schema + RLS + auto-linking triggers in `supabase/migrations/20260724_customer_accounts.sql`. **Requires manual setup before it works** — DLT registration, provider secrets, phone provider enabled: see `docs/ACCOUNTS-SETUP.md`.
