...
## 2026-09-17 - Sentinel: Timing safe equal leak prevention
**Vulnerability:** Use of `crypto.timingSafeEqual` with unequal length buffers throws an error or is circumvented by a short-circuit `&&` or early return in `api/_resend-webhook.js` and `api/verify-payment.js`, which causes a timing attack vulnerability that leaks the length of expected strings.
**Learning:** `crypto.timingSafeEqual` throws when buffer lengths are different, but protecting it with an early return creates a timing attack vulnerability that leaks the length.
**Prevention:** When verifying tokens or secrets of variable/unknown length, always execute a dummy constant-time comparison (e.g., `crypto.timingSafeEqual(a, a)`) before returning false on length mismatch to ensure execution time remains constant regardless of input length.
