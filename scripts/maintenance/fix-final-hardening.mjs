...
// manifest stores entries under `posts` and remains the single image mapping.
{
  ...
  if (fs.existsSync(manifestPath) && fs.existsSync(blogDir)) {
    ...
    const entries = Array.isArray(manifest?.posts) ? manifest.posts : (Array.isArray(manifest) ? manifest : Object.values(manifest || {}));
...
// does not provide one. This makes Track Order work regardless of the frontend
// and keeps the existing unique order_code index useful.
{
