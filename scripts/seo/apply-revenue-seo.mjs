...
const META = {
  '/index.html': { title: 'ODORSTRIKE Fabric Odor Spray for Clothes | Smelloff', description: 'Smelloff makes ODORSTRIKE, a pocket-sized fabric odor spray for clothes. ₹229 prepaid UPI in India. Not perfume. Not deodorant. Fabric only.', ogTitle: 'ODORSTRIKE Fabric Odor Spray for Clothes | Smelloff', ogDescription: 'ODORSTRIKE is a pocket-sized fabric odor spray for clothing. Not perfume. Not body deodorant. Made in Hyderabad, India. ₹229.' },
  '/odorstrike.html': { title: 'Fabric Odor Spray for Clothes India ₹229 | ODORSTRIKE', description: 'ODORSTRIKE is a 50ml fabric odor spray for clothes. Made for shirts, tees, jackets and other clothing. Not perfume. Not body deodorant. ₹229 in India.', ogTitle: 'ODORSTRIKE Fabric Odor Spray for Clothes ₹229 | Smelloff', ogDescription: 'A 50ml pocket fabric odor spray for clothing. Not perfume. Not deodorant. Built for shirts and everyday clothes in India.' },
  '/blog/index.html': { title: 'Fabric Odor Guides & Clothes Smell Fixes | Smelloff', description: 'Practical guides on fabric odor, sweat smell, clothes freshness and what actually helps. Science-led, India-first, written for real clothing problems.', ogTitle: 'Fabric Odor Guides & Clothes Smell Fixes | Smelloff', ogDescription: 'Science-led guides for sweat smell, clothing odor, fabric care and practical between-wash fixes.' },
};
...
function replaceTitle(html, value) { return html.replace(new RegExp('<title>[^<]*</title>', 'i'), `<title>${esc(value)}</title>`); }
...
function applyMeta(html, meta) {
  let out = replaceTitle(html, meta.title); out = replaceMeta(out, 'name', 'description', meta.description); out = replaceMeta(out, 'property', 'og:title', meta.ogTitle); out = replaceMeta(out, 'property', 'og:description', meta.ogDescription); out = replaceMeta(out, 'name', 'twitter:title', meta.ogTitle); out = replaceMeta(out, 'name', 'twitter:description', meta.ogDescription); return out;
}
...
function updateBlogIndex(html) {
  ...
  out = out.replace(/All Guides \(\d+\)/g, `All Guides (${total})`); out = out.replace(/id=\"latestCount\">\d+ guides/g, `id=\"latestCount\">${total} guides`); out = out.replace(/36 Science Guides/g, '50+ Fabric Guides'); out = out.replace(/Tested in 40°C Heat/g, 'India-first'); out = out.replace(/0% Oil \/ Fabric Safe/g, 'Fabric Only'); out = out.replace(/Peer-Reviewed Formulation/g, 'Source-led');
  const list = { '@context': 'https://schema.org', '@type': 'ItemList', itemListElement: collectBlogUrls().map((url, index) => ({ '@type': 'ListItem', position: index + 1, url: `https://smelloff.in${url}` })) };
  const itemListScript = `<script type="application/ld+json">\n${JSON.stringify(list)}\n</script>`; const itemListRe = /<script type="application\/ld\+json">[\s\S]*?"@type"\s*:\s*"ItemList"[\s\S]*?<\/script>/i; out = itemListRe.test(out) ? out.replace(itemListRe, itemListScript) : out.replace('</head>', `${itemListScript}\n</head>`);
  const start = '<!-- SF-REVENUE-STATIC-GUIDES:START -->'; const end = '<!-- SF-REVENUE-STATIC-GUIDES:END -->'; const block = `${start}\n<span id="smelloff-new-guides" hidden aria-hidden="true"></span>\n${staticGuideCards()}\n${end}`; const markerRe = new RegExp(`${start}[\\s\\S]*?${end}`, 'i'); if (markerRe.test(out)) out = out.replace(markerRe, block); else if (out.includes('<!-- Card 1 -->')) out = out.replace('<!-- Card 1 -->', `${block}\n<!-- Card 1 -->`); return out;
}
function updateProductPage(html) { let out = applyMeta(html, META['/odorstrike.html']); out = out.replace(/50ml mist that kills sweat smell, gym odor, and shirt stink in seconds —/gi, '50ml fabric odor-control mist for sweat and clothing odor —'); out = out.replace(/KILLS SWEAT SMELL IN SECONDS/gi, 'FABRIC ODOR CONTROL'); out = out.replace(/kills sweat smell, gym odor, and shirt stink in seconds/gi, 'helps neutralize sweat and clothing odor'); out = out.replace(/kills sweat smell in seconds/gi, 'helps neutralize sweat and clothing odor'); return out; }
function updateWinner(html, slug) { const item = WINNERS[slug]; if (!item) return html; let out = replaceTitle(html, item[0]); out = replaceMeta(out, 'name', 'description', item[1]); out = replaceMeta(out, 'property', 'og:title', item[0]); out = replaceMeta(out, 'property', 'og:description', item[1]); out = replaceMeta(out, 'name', 'twitter:title', item[0]); out = replaceMeta(out, 'name', 'twitter:description', item[1]); return out; }
function main() {
  const changed = []; const root = path.join(REPO, 'index.html'); const product = path.join(REPO, 'odorstrike.html'); const blog = path.join(BLOG_DIR, 'index.html');
  let html = fs.readFileSync(root, 'utf8'); let next = applyMeta(html, META['/index.html']); if (next !== html) { fs.writeFileSync(root, next); changed.push('index.html'); }
  html = fs.readFileSync(product, 'utf8'); next = updateProductPage(html); if (next !== html) { fs.writeFileSync(product, next); changed.push('odorstrike.html'); }
  html = fs.readFileSync(blog, 'utf8'); next = updateBlogIndex(html); if (next !== html) { fs.writeFileSync(blog, next); changed.push('blog/index.html'); }
  for (const slug of Object.keys(WINNERS)) { const file = path.join(BLOG_DIR, `${slug}.html`); if (!fs.existsSync(file)) continue; html = fs.readFileSync(file, 'utf8'); next = updateWinner(html, slug); if (next !== html) { fs.writeFileSync(file, next); changed.push(`blog/${slug}.html`); } }
  if (CHECK) { if (changed.length) { console.error(`Revenue SEO drift detected in ${changed.length} file(s):\n- ${changed.join('\n- ')}`); process.exit(1); } console.log('Revenue SEO: clean'); return; }
