...
(function () {
  ...
  function send(payload) {
    try {
      payload = payload || {};
      if (window.location.search) payload.path = payload.path || location.pathname;
  ...
  /* PDP checkout compatibility layer. Only /odorstrike is affected. */
  if (/^\/odorstrike\/?$/.test(location.pathname)) {
    ...
    if (typeof originalCreateSupabaseOrder === 'function') {
      window.createSupabaseOrder = function (opts) {
        opts = opts || {};
        var amountRupees = Number(opts.amountRupees) || 0;
        var payload = {
          email: opts.email || null,
          phone: opts.phone || '',
          items: Array.isArray(opts.items) ? opts.items : [],
          amount: Math.round(amountRupees * 100),
          payment_method: opts.paymentMethod || 'cod',
    ...
    if (typeof window.showSuccess === 'function') {
      ...
      window.showSuccess = function (orderId, method) {
        ...
        if (!pending || typeof pending.then !== 'function') return originalShowSuccess(orderId, method);
        pending.then(function () { originalShowSuccess(orderId, method); }).catch(function (error) {
          ...
          var btnText = document.getElementById('submitText'); if (btnText) btnText.textContent = method === 'cod' ? 'Place COD order · ₹289' : 'Open UPI app';
          if (typeof window.showError === 'function') window.showError(error && error.message ? error.message : 'We could not save your order. Please try again.');
        });
      };
    }
    if (typeof window.showUpiSuccess === 'function') {
      ...
      window.showUpiSuccess = function (orderId, total, upiLink) {
        var pending = window.__smelloffPdpOrderPromise;
        if (!pending || typeof pending.then !== 'function') return originalShowUpiSuccess(orderId, total, upiLink);
        pending.then(function () { originalShowUpiSuccess(orderId, total, upiLink); }).catch(function (error) {
          if (typeof window.hideLoadingScreen === 'function') window.hideLoadingScreen();
          var btn = document.getElementById('submitBtn'); if (btn) btn.disabled = false;
          if (typeof window.showError === 'function') window.showError(error && error.message ? error.message : 'We could not save your order. Please try again.');
        });
